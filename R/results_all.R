...
#' @param lfcShrink  shrink fold changes using \code{lfcShrink}, default normal, ashr or none.
#'        Use results_apeglm for apeglm shrinkage.  The MLE or unshrunken fold change is saved and added to the results as MLE_log2FC
...
results_all <- function(object, biomart,  vs="all", subset, relevel, alpha = 0.05,
 add_columns, trt, lfcShrink="normal", simplify=TRUE,  ...){
   message("Using adjusted p-value < ", alpha)
   if(lfcShrink == "apeglm") stop("apeglm is not supported, try results_apeglm instead")
   if(!lfcShrink %in% c("normal", "ashr")) lfcShrink <- "none"
   if(missing(trt)){
   ...
   if(lfcShrink != "none")  message("Adding shrunken fold changes to log2FoldChange, saving unshrunken in MLE_log2FC")
   for(i in seq_along( vs )){
       ...
       if(lfcShrink != "none"){
          ...
          res1 <-  DESeq2::lfcShrink(object, contrast=c( trt, contrast[1,i], contrast[2,i] ), res=res1, type = lfcShrink, quiet=TRUE)
          res1$MLE_log2FC <- mle_log2FC  
        # apeglm does not return all columns!
          n7 <- ncol(res1)
          res1 <- res1[, c(1,n7,2:(n7-1))]
       }

...
plot_biotypes <- function(x, n = 12, group = FALSE, stack = "percent", sort = TRUE,
  ...
  if (!group) {
    ...
    biotypes <- list(
      ...
      rRNA = "rRNA",
      `small RNA` = c(
        "miRNA", "scRNA", "snRNA", "snoRNA", "sRNA", "scaRNA",
        "vaultRNA"
      ),
      `other ncRNA` = c(
        "misc_RNA", "ribozyme", "non_coding", "sense_intronic",
      ...
      other = c(
        "TEC", "artifact", "3prime_overlapping_ncRNA",
        "nonsense_mediated_decay", "non_stop_decay", "retained_intron",
        "disrupted_domain", "ambiguous_orf"
      )
    )
    y <- dplyr::bind_rows(lapply(
      biotypes,
      function(x) tibble::tibble(name = x)
    ), .id = "feature")
